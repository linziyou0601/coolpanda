    ...
    with urllib.request.urlopen("https://opendata.cwb.gov.tw/api/v1/rest/datastore/F-D0047-089?Authorization=CWB-268DE0E2-66E8-4AE9-A0C3-B06F7EBB5E7A&elementName=Wx,T,CI,WeatherDescription,PoP6h") as url:
        ...
        for row in obj:
            if any((site in s) or (s in site) for s in [row['locationName'], row['locationName'][0:2]]):
                for i in range(len(row['weatherElement'][4]['time'])):
                    st_dt = datetime.strptime(row['weatherElement'][4]['time'][i]['startTime']+'.000000', '%Y-%m-%d %H:%M:%S.%f').astimezone(pytz.timezone("Asia/Taipei"))
                    ed_dt = datetime.strptime(row['weatherElement'][4]['time'][i]['endTime']+'.000000', '%Y-%m-%d %H:%M:%S.%f').astimezone(pytz.timezone("Asia/Taipei"))
                    if (not dt) or (dt > st_dt and dt < ed_dt): 
                        yield {
                            ...
                            'Description': row['weatherElement'][3]['time'][i]['elementValue'][0]['value'],
                            'PoP6h': row['weatherElement'][4]['time'][i]['elementValue'][0]['value']
                        }
        return ""
